...
class LuggageRegulations
    ...
    def bag_accumulator(b)

        return 1 if @regulations[b].has_key?('none')

        @regulations[b].inject(0) do |sum, (k,v)|

            acc = ( bag_accumulator(k) * v )

            @regulations[k].has_key?('none') ? sum += acc : sum += acc + v
...
p luggage.bag_accumulator('shiny_gold')
